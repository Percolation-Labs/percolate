...
class OpenAIResponseScheme(AIResponse):
    ...
    def parse(
        ...
        sid: str,
        model_name: str,
        streaming_callback: typing.Callable = False,
    ) -> AIResponse:
        """
        ...
        try:
            ...
            logger.warning(
                f"unexpected structure in OpenAI scheme message {response=} - caused the error {ex}"
            )
            raise
...
class AnthropicAIResponseScheme(AIResponse):
    ...
    def parse(
        ...
        if verbatim:
            ...
            verbatim = {"role": response["role"], "content": response["content"]}
...
def try_get_open_ai_key():
    ...
    try:
        params = db.execute(
            """select token from p8."LanguageModelApi" where token_env_key = 'OPENAI_API_KEY' and token is not null limit 1"""
        )
        if params:
...
class LanguageModel:
    ...
    def __init__(self, model_name: str):
        ...
        if self.params["token"] is None:
            ...
            if not self.params["token"]:
                raise Exception(
                    f"There is no token or token key configured for model {self.model_name} - you should add an entry to Percolate for the model using the examples in p8.LanguageModelApi"
                )
        """we use the env in favour of what is in the store"""
        env_token = os.environ.get(self.params["token_env_key"])
        self.params["token"] = (
            env_token
            if env_token is not None and len(env_token)
    ...
    def parse(
        ...
        streaming_callback = (
            ...
            if context and context.streaming_callback
            else None
        )
        ...
        try:
            ...
            if self.params.get("scheme") == "google":
                return GoogleAIResponseScheme.parse(
                    response,
                    sid=sid,
            ...
            if self.params.get("scheme") == "anthropic":
                return AnthropicAIResponseScheme.parse(
                    response,
                    sid=sid,
                    model_name=self.model_name,
                    streaming_callback=streaming_callback,
                )
            return OpenAIResponseScheme.parse(
                response,
                sid=sid,
                model_name=self.model_name,
                streaming_callback=streaming_callback,
    ...
    def __call__(
        ...
        try:
            response = self._call_raw(
                messages=messages, functions=functions, context=context
            )
    ...
    def ask(
        ...
        **kwargs,
        ...
        return self.__call__(
            MessageStack(question, system_prompt=system_prompt),
            functions=functions,
            context=context,
            **kwargs,
        )
    ...
    def get_embedding(self, text: str) -> typing.List[float]:
        ...
        return get_embedding(
            text=text, model=model, api_key=self.params["token"], scheme=scheme
        )
    ...
    def get_embeddings(
        ...
        return get_embeddings(
            texts=texts, model=model, api_key=self.params["token"], scheme=scheme
        )
    ...
    def call_api_simple(
        ...
        **kwargs,
        ...
        if not token or len(token) == 0:
            raise Exception(
                f"There is no API KEY in the env or database for model {self.model_name} - check ENV {params.get('token_env_key')}"
            )
        headers = {
        ...
        if params["scheme"] == "anthropic":
            ...
            headers["anthropic-version"] = self.params.get(
        ...
        if params["scheme"] == "google":
            url = (
                f"{url}?key={token}"
                if not is_streaming
                else f"{url.replace('generateContent','streamGenerateContent')}?alt=sse&key={token}"
            )
        data = {
            "model": params["model"],
            "messages": [
        ...
        if params["scheme"] == "anthropic":
            data = {
                ...
                "messages": [
                    ...
                    *[
                        MessageStackFormatter.adapt_tool_response_for_anthropic(d)
        ...
        if params["scheme"] == "google":
            data_content = [
                MessageStackFormatter.adapt_tool_response_for_google(d)
                for d in data_content
                if d
            ]
            optional_tool_config = (
                {}
            )  # { "tool_config": {   "function_calling_config": {"mode": "ANY"}  }  } #this seems to confuse the googs
            data = {
                "contents": [
            ...
            if (
                not data_content
                or "functionResponse" not in data_content[-1]["parts"][0]
            ):
        ...
        if response.status_code not in [200, 201]:
            logger.error(
                f"API request failed: {response.status_code=}  {response.content}"
            )
            ...
            if response.status_code == 400:
                try:
                    ...
                    if "verified to stream" in error_msg:
                        raise Exception(
                            f"GPT-5 streaming requires organization verification. Model: {self.model_name}. Error: {error_msg}"
                        )
                    else:
                        raise Exception(
                            f"Bad request (400) for model {self.model_name}: {error_msg}"
                        )
                except json.JSONDecodeError:
                    raise Exception(
                        f"Bad request (400) for model {self.model_name}: {response.text}"
                    )
            else:
                raise Exception(
                    f"API request failed with status {response.status_code} for model {self.model_name}: {response.text}"
                )
    ...
    def parse_ai_response(
        self, partial: dict, usage: dict, ctx: CallingContext
    ) -> AIResponse:
        """
    ...
    def stream_with_proxy(self, messages, functions, context):
        ...
        request = R(
            model=self.model_name, messages=message_data, stream=True, tools=functions
        )
