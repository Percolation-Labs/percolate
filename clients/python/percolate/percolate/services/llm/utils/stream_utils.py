                                ...
                                if "index" in tool_call:
                                    ...
                                    if (
                                        ...
                                        if tool_index > len(tool_calls) - 1:
                                            raise Exception(
                                                f"the index {tool_index} was expected in {tool_calls} but it was not found - {tool_call=} {collected_lines_debug=}"
                                            )

                                        tool_calls[tool_index]["function"][
                                            "arguments"
            ...
            if decoded_line and decoded_line != "[DONE]":
                try:
                    ...
                    # Handle message delta and stop reason at the end
                        ...
                        output_tokens = json_data.get("usage", {}).get(
                            "output_tokens", 0
                        )
...
def request_openai(messages, functions):
    ...
    data = {"model": "gpt-4o-mini", "messages": messages, "tools": functions}
...
def request_google(messages, functions):
    ...
    data = {"contents": [m for m in messages if m["role"] != "system"]}
        ...
        try:
            ...
            for item in self.g():
                ...
                if isinstance(item, AIResponse):
                    ...
                    #     logger.debug(
                    #         f"content preview: {item.content[:100] if item.content else 'None'}"
                    #     )
