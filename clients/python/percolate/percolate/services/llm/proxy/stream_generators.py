...
def convert_chunk_to_target_scheme(chunk: dict, target_scheme: str) -> dict:
    ...
    if target_scheme == "openai":
        ...
        logger.warning(
            f"Unknown target scheme: {target_scheme}, falling back to OpenAI format"
        )
        return chunk
...
def stream_with_buffered_functions(
    ...
    for line in response.iter_lines(decode_unicode=True):
        ...
        try:
            ...
            # print(1, "----")
            # print(canonical_chunk, canonical_chunk.keys())
            if "usage" in canonical_chunk and canonical_chunk["usage"]:
                ...
                if relay_usage_events:
                    target_chunk = convert_chunk_to_target_scheme(
                        canonical_chunk, target_scheme
                    )
            ...
            # Buffer tool calls
                ...
                for tool_delta in delta["tool_calls"]:
                    if "id" in tool_delta:
                        ...
                        t["function"]["arguments"] += tool_delta["function"][
                            "arguments"
                        ]
                ...
                target_chunk = convert_chunk_to_target_scheme(
                    consolidated_chunk, target_scheme
                )
                target_line = f"data: {json.dumps(target_chunk)}\n\n"
    ...
    **kwargs,
    ...
    try:
        ...
        response = requests.post(
            api_url, headers=headers, data=json.dumps(api_data), stream=True
        )
...
def collect_stream_to_response(
    stream_iterator: typing.Union[
        typing.Generator[typing.Tuple[str, dict], None, None], "LLMStreamIterator"
    ],
    source_scheme: str = "openai",
    target_scheme: str = "openai",
    model: str = "unknown",
    ...
    if isinstance(stream_iterator, LLMStreamIterator):
        ...
        if not stream_iterator._is_consumed:
            try:
                ...
                return {"skipped": "p8.AIResponse", "tokens": 0, "error": str(e)}
                ...
                if target_scheme == "anthropic":
                    return AnthropicResponse(
                        ...
                        content=[
                            {
                                "type": "text",
                                "text": error_response["choices"][0]["message"][
                                    "content"
                                ],
                            }
                        ],
    ...
    if isinstance(stream_iterator, dict) and (
        "skipped" in stream_iterator or "error" in stream_iterator
    ):
...
def request_non_streaming_response(
    ...
    **kwargs,
    ...
    stream_gen = request_stream_from_model(
        request, context, target_scheme=target_scheme, **kwargs
    )
