...
from percolate.api.routes.auth import (
    get_current_token,
    hybrid_auth,
    require_user_auth,
...
def get_messages_by_role_from_request(
    request: CompletionsRequestOpenApiFormat, metadata: dict
):
    """"""
    ...
    if request.messages:
        ...
        if is_audio := metadata.get("is_audio"):
            ...
            logger.info(
                f"is_audio set so we will assume ALL user content is based 64 audio"
            )
        ...
        for msg in request.messages:
            ...
            if msg.role == "system" and isinstance(msg.content, str):
                ...
                if is_audio:
                    try:
                        logger.info(
                            f"transcribing supposed based 64 encoded audio content - {last_user_content[:10]}...{last_user_content[-10:]}"
                        )
                        last_user_content = audio_to_text(last_user_content)["text"]
                        metadata["transcribed_audio"] = last_user_content
                    except:
                        logger.warning(
                            f"Failed to transcribe and we will let the language model handle it or not for now - {traceback.format_exc()}"
                        )
...
def handle_agent_request(
    ...
    language_model_class=LanguageModel,
    ...
    if agent_model_name:
        ...
        if "." not in agent_model_name:
            logger.warning(
                f"Agent name '{agent_model_name}' does not have a namespace. Will try to load as is, but this might cause issues."
            )
...
def handle_openai_request(
    ...
    if is_audio:
        """TODO: we only support this on the open ai handler for now as its experimental - not sure how we want to deal with transcription inline"""
        logger.info(
            f"is_audio set so we will assume ALL user content is based 64 audio. {metadata=}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid model: {model_name}. Error: {str(e)}"
        )
...
def handle_anthropic_request(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calling Anthropic LLM: {str(e)}"
        )
...
def handle_google_request(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calling Google LLM: {str(e)}"
        )
...
def map_delta_to_canonical_format(data, dialect, model):
    ...
    return StreamingCompletionsResponseChunk.map_to_canonical_format(
        data, dialect, model
    )
        ...
        for chunk in response.iter_lines():
            ...
            if from_dialect and from_dialect != "openai":
                ...
                if json_data and json_data[0] == "{":
                    ...
                    canonical_data = map_delta_to_canonical_format(
                        json.loads(json_data), from_dialect, model
                    )
...
def extract_metadata(request, params=None):
    ...
    if hasattr(request, "chat_id") and request.chat_id:
        ...
        logger.info(
            f"[CHAT DEBUG] Using top-level chat_id {request.chat_id} as session_id"
        )
    ...
    # 2. Metadata chat_id is next priority
        ...
        logger.info(
            f"[CHAT DEBUG] Using OpenWebUI metadata.chat_id {metadata['chat_id']} as session_id"
        )

    # 3. Params chat_id is lowest priority
    elif (
        params
        and "metadata" in params
        and isinstance(params["metadata"], dict)
        and "chat_id" in params["metadata"]
        and params["metadata"]["chat_id"]
    ):
        metadata["chat_id"] = params["metadata"]["chat_id"]
        metadata["session_id"] = params["metadata"]["chat_id"]
        chat_id_source = "params"
        logger.info(
            f"[CHAT DEBUG] Using OpenWebUI chat_id {params['metadata']['chat_id']} from params as session_id"
        )
    ...
    logger.info(
        f"[CHAT DEBUG] Final extracted metadata with chat_id from {chat_id_source or 'none'}: {metadata}"
    )
...
def audit_request(
    ...
    if metadata:
        ...
        session_metadata = {
            k: v for k, v in metadata.items() if v and k != "session_id"
        }
    else:
        session_metadata = {}
...
async def completions(
    ...
    user_id: Optional[str] = Query(
        None, description="ID of the end user making the request"
    ),
    session_id: Optional[str] = Query(
        None, description="ID for grouping related interactions"
    ),
    channel_id: Optional[str] = Query(
        None, description="ID of the channel where the interaction happens"
    ),
    channel_type: Optional[str] = Query(
        None, description="Type of channel (e.g., slack, web, etc.)"
    ),
    api_provider: Optional[str] = Query(
        None, description="Override the default provider"
    ),
    is_audio: Optional[bool] = Query(
        False, description="Client asks to decoded base 64 audio using a model"
    ),
    device_info: Optional[str] = Query(
        None,
        description="Device info Base64 encoded with arbitrary parameters such as GPS",
    ),
    ...
    logger.info(
        f"{effective_user_id}, {session_id}, role_level={user_role_level}, auth method: {'session' if auth_user_id else 'bearer'}"
    )
    ...
    try:
        ...
        if stream_mode:
            ...
            media_type = (
                "text/event-stream"  # if stream_mode == 'sse' else "text/plain"
            )
        ...
        logger.warning(traceback.format_exc())
        raise HTTPException(
            status_code=400, detail=f"Something happened that should not have happened."
        )
...
async def agent_completions(
    ...
    effective_user_id = auth_user_id or user_id
    logger.info(
        f"Agent completions request - User: {effective_user_id}, Agent: {agent_name}, Model: {request.model}, Session: {session_id}, Role Level: {user_role_level}, Auth Method: {'session' if auth_user_id else 'bearer'}"
    )
    ...
    try:
        ...
        response = handle_agent_request(
            request, params, request.model, agent_model_name=agent_name
        )
...
async def anthropic_completions(
    ...
    return await completions(
        openai_request, background_tasks, auth_user_id=auth_user_id
    )
...
class SimpleAskRequest(BaseModel):
    ...
    model: Optional[str] = Field(
        None, description="The language model to use - Percolate defaults to GPT models"
    )
    question: str = Field(..., description="A simple question to ask")
    agent: Optional[str] = Field(
        None,
        description="The configured agent - the Percolate agent will be used by default to answer generic questions",
    )
    max_iteration: Optional[int] = Field(
        3, description="The agent runs loops - for simple ask fewer is better"
    )
...
async def ask(
    ...
    try:
        ...
        background_tasks.add_task(audit_request, request, result, {"agent": agent_name})
