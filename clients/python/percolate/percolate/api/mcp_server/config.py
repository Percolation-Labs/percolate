...
class MCPSettings(BaseSettings):
    ...
    mcp_server_name: str = Field(
        default="percolate-mcp", description="Name of the MCP server for identification"
    )
...
4. Combine tools: find functions with search, then execute with function_eval""",
    ...
    api_endpoint: str = Field(
        default_factory=lambda: from_env_or_project(
            "P8_API_ENDPOINT",
            from_env_or_project("P8_TEST_DOMAIN", "https://api.percolationlabs.ai"),
        ),
    ...
    use_api_mode: bool = Field(
        default_factory=lambda: from_env_or_project("P8_USE_API_MODE", "true").lower()
        == "true",
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: from_env_or_project(
            "P8_PG_PASSWORD",
            from_env_or_project(
                "P8_API_KEY", from_env_or_project("P8_TEST_BEARER_TOKEN", None)
            ),
        ),
    ...
    user_email: Optional[str] = Field(
        default_factory=lambda: from_env_or_project(
            "P8_USER_EMAIL", from_env_or_project("X_User_Email", None)
        ),
        description="User email for authentication context. Uses P8_USER_EMAIL (preferred) or X_User_Email from environment. Can be embedded in JWT token instead.",
    )
    ...
    user_groups: Optional[List[str]] = Field(
        default_factory=lambda: (
            from_env_or_project("P8_USER_GROUPS", "").split(",")
            if from_env_or_project("P8_USER_GROUPS", "")
            else None
        ),
    ...
    role_level: int = Field(
        default_factory=lambda: int(
            from_env_or_project("P8_ROLE_LEVEL", str(SYSTEM_USER_ROLE_LEVEL))
        ),
    ...
    default_agent: str = Field(
        ...
        json_schema_extra={
            "env_var": "P8_DEFAULT_AGENT"
        },  # Indicate this should be read from environment
    )
    ...
    default_vision_model: str = Field(
        default_factory=lambda: from_env_or_project(
            "P8_DEFAULT_VISION_MODEL", "gpt-4o"
        ),
    ...
    agent_max_depth: int = Field(
        default=3, description="Default maximum recursion depth for agent operations"
    )

    agent_allow_help: bool = Field(
        default=True, description="Whether agents can access help functions"
    )

    # Runtime flags
    is_desktop_extension: bool = Field(
        default_factory=lambda: os.getenv("P8_MCP_DESKTOP_EXT", "false").lower()
        == "true",
...
def _load_master_prompt_from_db() -> Optional[str]:
    ...
    try:
        ...
        query = """
        SELECT value FROM settings WHERE name = 'system_prompt'
        LIMIT 1
        """
        ...
        logging.getLogger(__name__).warning(
            f"Could not load MASTER_PROMPT from database: {traceback.format_exc()}"
        )
