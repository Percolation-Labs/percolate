...
class ResourceHandler:
    ...
    def extract_content(
        self, file_data: Any, file_type: str, mode: str = "simple", **kwargs
    ) -> str:
        ...
        if isinstance(file_data, str):
            ...
            for sheet_name, df in file_data.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"Error converting sheet '{sheet_name}' to string: {e}"
                    )
...
class ImageResourceHandler(ResourceHandler):
    ...
    def extract_content(
        ...
        if mode == "simple":
            ...
            try:
                from percolate.services.llm.ImageInterpreter import (
                    get_image_interpreter,
                )
...
class AudioResourceHandler(ResourceHandler):
    ...
    def extract_content(
        ...
        if mode == "simple":
            ...
            logger.warning(
                f"Simple mode not supported for audio file {file_name}. Use extended mode for transcription."
            )
...
class ResourceChunker:
    ...
    def __init__(
        ...
        read_bytes_function: Optional[Callable] = None,
    ):
    ...
    def _get_transcription_service(self):
        ...
        if self._transcription_service is None:
            from percolate.services.llm.TranscriptionService import (
        ...
        try:
            ...
            if file_data is None:
                if self.read_function:
                    ...
                    raise ValueError(
                        "No read_function provided and no file_data supplied"
                    )
            ...
            if parsing_mode == "simple":
                content = handler.extract_content(
                    file_data, file_type, mode="simple", file_name=file_name, uri=uri
                )
            else:  # extended
                content = handler.extract_content(
                    file_data, file_type, mode="extended", file_name=file_name, uri=uri
                )
    ...
    def _extract_file_info(self, uri: str) -> Dict[str, str]:
        ...
        return {"name": file_name, "type": file_type, "extension": ext}
    ...
    def _chunk_media_resource(
        ...
        max_transcription_size = (
            10 * 1024 * 1024
        )  # 10MB limit (reduced for memory efficiency)
        chunk_duration_seconds = (
            5 * 60
        )  # 5 minutes per chunk (reduced for memory efficiency)
        ...
        try:
            ...
            audio_chunks = self._prepare_audio_chunks(
                temp_path, file_size, max_transcription_size, chunk_duration_seconds
            )

            # Transcribe all audio chunks
            all_transcriptions, total_duration = self._transcribe_audio_chunks(
                audio_chunks, temp_path
            )

            # Check if we have any successful transcriptions
            if not all_transcriptions:
                logger.warning(
                    "No transcriptions were successful. Cannot create resources."
                )
                return []
    ...
    def _validate_media_processing_requirements(
        self, file_type: str, parsing_mode: str
    ) -> None:
    ...
    def _download_media_file(self, uri: str) -> Tuple[str, int]:
        ...
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=Path(uri).suffix
        ) as temp_file:
    ...
    def _prepare_audio_chunks(
        self, file_path: str, file_size: int, max_size: int, chunk_duration: int
        ...
        if not transcription_service.supports_file_type(file_path):
            raise ValueError(
                f"File type {Path(file_path).suffix} not supported for transcription"
            )

        # Determine if we need to chunk the audio file
        if file_size <= max_size:
            logger.info(
                "File size within transcription limits, processing as single file"
            )
            return [(file_path, 0, None)]  # (path, start_time, end_time)
        else:
            logger.info(
                f"File size ({file_size / (1024*1024):.1f}MB) exceeds limit, chunking audio"
            )
            return self._chunk_large_audio_file(file_path, max_size, chunk_duration)

    def _transcribe_audio_chunks(
        self, audio_chunks: List[Tuple[str, float, Optional[float]]], original_path: str
        ...
        for i, (chunk_path, start_time, end_time) in enumerate(audio_chunks):
            ...
            try:
                ...
                transcription, confidence = transcription_service.transcribe_file(
                    chunk_path
                )
                ...
                if start_time is not None and end_time is not None:
                    ...
                    timestamp_text = (
                        f"[{start_time:.1f}s - {end_time:.1f}s]: {transcription}"
                    )
        ...
        if len(audio_chunks) > 0 and audio_chunks[0][0] != original_path:
            ...
            if os.path.exists(chunk_dir) and chunk_dir.startswith(
                tempfile.gettempdir()
            ):
                try:
                    import shutil
                    ...
                    logger.warning(
                        f"Failed to clean up chunk directory {chunk_dir}: {e}"
                    )
    ...
    def _create_transcription_resources(
        ...
        full_transcription = "\n\n".join(
            [t["timestamped_text"] for t in transcriptions]
        )
        average_confidence = (
            sum(t["confidence"] for t in transcriptions) / len(transcriptions)
            if transcriptions
            else 0.0
        )

        logger.info(
            f"Complete transcription: {len(full_transcription)} characters from {len(transcriptions)} chunks"
        )
        ...
        logger.info(
            f"Chunking transcription text with size={chunk_size}, overlap={chunk_overlap}"
        )
        text_chunks = self._create_text_chunks(
            full_transcription, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )

        # Log the number of chunks created
        logger.info(
            f"Created {len(text_chunks)} text chunks from transcription ({len(full_transcription)} chars)"
        )
        ...
        logger.info(
            f"Created {len(resources)} transcription resources for {file_info['name']}"
        )
        return resources
    ...
    def _chunk_large_audio_file(
        self, audio_path: str, max_size_bytes: int, chunk_duration_seconds: int
    ) -> List[tuple]:
        """
        ...
        try:
            ...
            logger.info(
                f"Audio duration: {duration_seconds:.1f}s, file size: {file_size / (1024*1024):.1f}MB"
            )
            ...
            chunks_by_size = max(
                1,
                file_size // max_size_bytes + (1 if file_size % max_size_bytes else 0),
            )
            chunks_by_duration = max(
                1,
                int(duration_seconds // chunk_duration_seconds)
                + (1 if duration_seconds % chunk_duration_seconds else 0),
            )
            ...
            logger.info(
                f"Splitting into {num_chunks} chunks of ~{chunk_duration_ms/1000:.1f}s each"
            )
            ...
            for i in range(num_chunks):
                ...
                logger.info(
                    f"Created chunk {i+1}: {start_time:.1f}s - {end_time:.1f}s ({os.path.getsize(chunk_path) / (1024*1024):.1f}MB)"
                )
            ...
            raise ImportError(
                "pydub library is required for large audio file processing"
            )
...
def create_resource_chunker(fs=None):
    ...
    if fs is None:
        ...
        return ResourceChunker(read_function=fs.read, read_bytes_function=fs.read_bytes)
