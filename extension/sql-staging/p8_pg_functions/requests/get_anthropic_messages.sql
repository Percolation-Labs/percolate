-- FUNCTION: p8.get_anthropic_messages(text, text, text)
...
BEGIN
    ...
    message_data AS (
        ...
        SELECT 'system' AS role,  json_build_array(json_build_object(
			'text',agent_or_system_prompt,
			'type', 'text'
		) ) AS content
		   WHERE agent_or_system_prompt IS NOT NULL
        UNION ALL
        SELECT 'user' AS role, json_build_array(json_build_object(
			'text',question,
			'type', 'text'
		)) AS content
			WHERE question IS NOT NULL
        UNION ALL
        SELECT  case when role = 'tool' then 'user' else role end as role,
		 CASE 
			...
			--TODO: currently only supports one tool call
				jsonb_build_array(
					 jsonb_build_object(
						...
						'id', tool_calls->0->'id',
						'input', (tool_calls->0->'function'->> 'arguments')::json,
						'type', 'tool_use'
					)
				)::JSON						
			ELSE 
...
ALTER FUNCTION p8.get_anthropic_messages(text, text, text)
    OWNER TO postgres;
