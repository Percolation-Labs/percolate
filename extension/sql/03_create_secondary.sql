...
INSERT INTO p8."ModelField" (name, id, entity_name, field_type, embedding_provider, description, is_key) VALUES
 ...
 ('query', '5f781f67-29c1-c21c-2b4c-f891a1455bf4', 'p8.Session', 'str', NULL, 'the question or context that triggered the session', False),
 ...
 ('parent_session_id', '15dc76f1-d9f2-da78-40e5-e5f2b27ecf4d', 'p8.Session', 'UUID', NULL, NULL, False),
...
```{''''description'''': ''''Tracks groups if session dialogue'''', ''''properties'''': {''''id'''': {''''anyOf'''': [{''''format'''': ''''uuid'''', ''''type'''': ''''string''''}, {''''type'''': ''''string''''}], ''''title'''': ''''Id''''}, ''''query'''': {''''anyOf'''': [{''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''the question or context that triggered the session'''', ''''title'''': ''''Query''''}, ''''user_rating'''': {''''anyOf'''': [{''''type'''': ''''number''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''We can in future rate sessions to learn what works'''', ''''title'''': ''''User Rating''''}, ''''agent'''': {''''default'''': ''''Percolate always expects an agent but we support passing a system prompt which we treat as anonymous agent'''', ''''title'''': ''''Agent'''', ''''type'''': ''''string''''}, ''''parent_session_id'''': {''''anyOf'''': [{''''format'''': ''''uuid'''', ''''type'''': ''''string''''}, {''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': ''''A session is a thread from a question+prompt to completion. We maw span child sessions'''', ''''title'''': ''''Parent Session Id''''}, ''''thread_id'''': {''''anyOf'''': [{''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''An id for a thread which can contain a number of sessions'''', ''''title'''': ''''Thread Id''''}, ''''channel_id'''': {''''anyOf'''': [{''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''The channel through which the user came. It could be a messaging platform channel or a system division'''', ''''title'''': ''''Channel Id''''}, ''''channel_type'''': {''''anyOf'''': [{''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''The channel type'''', ''''title'''': ''''Channel Type''''}, ''''metadata'''': {''''anyOf'''': [{''''type'''': ''''object''''}, {''''type'''': ''''null''''}], ''''description'''': ''''Arbitrary metadata'''', ''''title'''': ''''Metadata''''}, ''''session_completed_at'''': {''''anyOf'''': [{''''format'''': ''''date-time'''', ''''type'''': ''''string''''}, {''''type'''': ''''null''''}], ''''default'''': None, ''''description'''': ''''An audit timestamp to write back the completion of the session - its optional as it should manage the last timestamp on the AI Responses for the session'''', ''''title'''': ''''Session Completed At''''}}, ''''required'''': [''''id''''], ''''title'''': ''''Session'''', ''''type'''': ''''object''''}``` 

# Functions
 ```
None```
            ', '{"description": "Tracks groups if session dialogue", "properties": {"id": {"anyOf": [{"format": "uuid", "type": "string"}, {"type": "string"}], "title": "Id"}, "query": {"anyOf": [{"type": "string"}, {"type": "null"}], "default": null, "description": "the question or context that triggered the session", "title": "Query"}, "user_rating": {"anyOf": [{"type": "number"}, {"type": "null"}], "default": null, "description": "We can in future rate sessions to learn what works", "title": "User Rating"}, "agent": {"default": "Percolate always expects an agent but we support passing a system prompt which we treat as anonymous agent", "title": "Agent", "type": "string"}, "parent_session_id": {"anyOf": [{"format": "uuid", "type": "string"}, {"type": "string"}, {"type": "null"}], "default": "A session is a thread from a question+prompt to completion. We maw span child sessions", "title": "Parent Session Id"}, "thread_id": {"anyOf": [{"type": "string"}, {"type": "null"}], "default": null, "description": "An id for a thread which can contain a number of sessions", "title": "Thread Id"}, "channel_id": {"anyOf": [{"type": "string"}, {"type": "null"}], "default": null, "description": "The channel through which the user came. It could be a messaging platform channel or a system division", "title": "Channel Id"}, "channel_type": {"anyOf": [{"type": "string"}, {"type": "null"}], "default": null, "description": "The channel type", "title": "Channel Type"}, "metadata": {"anyOf": [{"type": "object"}, {"type": "null"}], "description": "Arbitrary metadata", "title": "Metadata"}, "session_completed_at": {"anyOf": [{"format": "date-time", "type": "string"}, {"type": "null"}], "default": null, "description": "An audit timestamp to write back the completion of the session - its optional as it should manage the last timestamp on the AI Responses for the session", "title": "Session Completed At"}}, "required": ["id"], "title": "Session", "type": "object"}', NULL)
    ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, category = EXCLUDED.category, description = EXCLUDED.description, spec = EXCLUDED.spec, functions = EXCLUDED.functions;
-- ------------------
