...
INSERT INTO p8."ModelField"(name,id,entity_name,field_type,embedding_provider,description,is_key) VALUES
 ...
 ('thread_id', '45e15ffc-01b7-f83e-9b29-4a988ea8b685', 'p8.Session', 'str', NULL, 'An id for a thread which can contain a number of sessions', NULL),
 ('channel_id', 'de145ff6-182c-e8c9-d2b6-53f807e84655', 'p8.Session', 'str', NULL, 'The channel through which the user came. It could be a messaging platform channel or a system division', NULL),
 ('channel_type', '24d30b26-6236-31f5-58b9-75d99a6244d3', 'p8.Session', 'str', NULL, 'The channel type', NULL),
 ('metadata', '83589148-e6cf-7d27-19e1-c97c974d537e', 'p8.Session', 'dict', NULL, 'Arbitrary metadata', NULL),
 ('session_completed_at', '4b372d12-fcbd-25df-89b4-749350663cd2', 'p8.Session', 'datetime', NULL, 'An audit timestamp to write back the completion of the session - its optional as it should manage the last timestamp on the AI Responses for the session', NULL)
        ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name,entity_name=EXCLUDED.entity_name,field_type=EXCLUDED.field_type,embedding_provider=EXCLUDED.embedding_provider,description=EXCLUDED.description,is_key=EXCLUDED.is_key   ;
-- ------------------
